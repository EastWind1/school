$(function(){
	...
	function getData(){
				...
				if(len >0){
					$.each(data.rows,function(i,val){
						html+="<tr>" +
								...
								"<td style='cursor:pointer' onclick='update(\""+val.id+"\",\""+val.roleName+"\")'>"+
								"<span class='checkDetailsBtn'>修改</span></td></tr>";
					})
				}else{
   	...
   	$("#addbutton").on("click",function(){
   		window.location.href="media/power/roleMenuEdit.jsp";
   	})
...
function update(roleId,roleName){
	...
	window.location.href="media/power/roleMenuUpdate.jsp?roleId="+roleId;
}
