...
<body>
	...
	<!-- <p class="lsfz_titimg" style="margin-top:15px;"><img src="weixin/images/title_choujiang.png" /></p> -->	
		...
		<p id="cleandata" class="lsfz_titimg foot">绿色方舟第3季南区半决赛有奖竞猜<br/>本场竞猜将抽出20个幸运儿送出精美礼品</p> 
 	</div>
 	</div>
  ...
  <script src="weixin/js/jweixin-1.0.0.js"></script>
   ...
   $("#cleandata").on("click",function(){
    	window.localStorage.setItem("shakeZj","0");
    	window.localStorage.setItem("shakeCs","0");	
    	alert("本地数据已清除");
    })
    
    var wzjlast=window.localStorage.getItem("wzj");
    var SHAKE_THRESHOLD = 120;//控制摇一摇幅度
    var last_update = 0;
    var x = y = z = last_x = last_y = last_z = 0;
    var shakeZj=window.localStorage.getItem("shakeZj");
    var shakeCs=window.localStorage.getItem("shakeCs");
      ...
      function deviceMotionHandler(eventData) {
        ...
        var curTime = new Date().getTime();
    	 var diffTime = curTime - last_update;
    		...
    		if (speed > SHAKE_THRESHOLD && (curTime - last_update) > 1500) {
				   last_update = curTime;	    
		            last_x = x;
		            last_y = y;
		            last_z = z;
  ...
  /*  
    ...
    function deviceMotionHandler(eventData) {
        ...
        if ((curTime - last_update) > 1500) {//摇一摇时间间隔
        count-=1;
            var diffTime = curTime - last_update;
          	last_update = curTime;
            x = acceleration.x;
	...
	function getcount(){
   	...
   	//摇一摇获取随机数
   	 function doResult(){
   	 	...
   	 	var rando=1;
   	 	if(shakezjcs == 10){
   	 		addShake("0");//0代表不中奖，已经有两位中奖，则一直不中奖
   	 	}else{
    ...
    //	add.addParams("customerName",memberName);
    	...
    	add.ajax(getIP()+"gameshakeprize.action?addlsfzPrizeThree",function(data){
    		if(data.shakeZj == "1"){//中奖
    			...
    			//alert("是否中奖："+state);
    			$(".yyyyyyy img").attr("src","weixin/images/yaoyiyao_choujiang.png");
    			window.localStorage.setItem("shakeZj","1");
    			$("#msg").html("恭喜您中奖了！客服人员会尽快与您联系。");
    			$("#msgcjcs").html("");
    		}else{
    			//播放未中奖音乐
    		...
    		//更新本地摇奖次数
    		window.localStorage.setItem("shakeCs",data.shakeCs);
