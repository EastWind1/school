...
<html>
  ...
  <body>
	...
	<script>
		...
		$(function(){
			var p=$.Params.create();
			p.addParams("filed1","vote"); 
			p.ajax(getIP()+"eventgateam.action?list",function(data){
				if(data.success){
					var len=data.total;
					var html="";
					if(len>0){
						html1="<li class='titlefz' style='text-align:center'>南区半决赛冠军竞猜-甲组</li>";
						html2="<li class='titlefz' style='text-align:center'>南区半决赛冠军竞猜-乙组</li>";
						html3="";
						html4="";
			...
			$("#tp_subBtn").on("click",function(){
				...
				$("li[selected]").each(function(){
					...
					var param="{\"id\":\""+id+"\",\"openId\":\""+openId+"\",\"teamId\":\""+teamId+"\",\"groupName\":\""+groupName+"\",\"createTime\":\""+createTime+"\",\"filed1\":\"vote\"}";
					params.push(param);
				});
				if(params.length<2){
					alert("甲组、乙组各投一票！");
		    	...
		    	add.ajax(getIP()+"eventgavote.action?addMoreVote",function(data){
		    		if(data.success){
		    			window.location.href="weixin/tp_fz/votedList.jsp";
		    		}else{
		    			alert(data.msg);
		    			return;
		    		}
		...
		wx.ready(function () {
		  wx.checkJsApi({
		    ...
		    wx.onMenuShareTimeline({
			    title: '绿色方舟第3季南区半决赛有奖竞猜', //分享标题
		      	link: 'http://weixin.yunicu.com/ychz/weixin/tp_fz/vote.jsp',
