...
if(memberId==""){
	window.location.href=getIP()+"PC/newIndex.jsp";
 	...
 	$(function(){
 		...
 		//summary.addParams("filed5","1");//查询认证的专家
		...
		$("#sumSubmit_ryjc,#sumSubmit_sysjc,#sumSubmit_yxxjc,#sumSubmit_other").live("click",function(){
			...
			$("#"+name+"").val(uuid);
			$("#modalcont").html("<div style='overflow:hidden;'><img src='' alt='error' style='display:none;width: 70%;height: 255px;margin:0 auto;' /></div><form id='midicalRecord' method='post' enctype='multipart/form-data'>上传图片："+
