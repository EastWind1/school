package com.ian.media.util.weixin.util;
...
import com.ian.media.util.weixin.message.resp.Article;
import com.ian.media.util.weixin.message.resp.MessArticle;
import com.ian.media.util.weixin.message.resp.Music;
import com.ian.media.util.weixin.pojo.*;
...
public class AdvancedUtil {
	 ...
	 * 上传图文推送消息
	 */
	public static WeixinMedia uploadMessMessage(String accessToken, List<MessArticle> articleList) {
		String jsonMsg = "{\"articles\":%s}";
		jsonMsg = String.format(jsonMsg,JSONArray.fromObject(articleList));
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				log.info("推送消息上传成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				log.error("推送消息上传失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	 ...
	 * @return
	 */
	public static String makeNewsMassMessage(int groupid,String mediaId) {
		String jsonMsg = "{\"filter\":{\"is_to_all\":false,\"group_id\":%d},\"mpnews\":{\"media_id\":\"%s\"},\"msgtype\":\"mpnews\",\"end_ignore_reprint\":0}";
	...
	public static boolean sendCustomMessage(String accessToken, String jsonMsg) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				log.info("消息发送成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				log.error("消息发送失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	...
	public static boolean sendAllMessage(String accessToken, String jsonMsg) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				log.info("推送发送成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				log.error("推送发送失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	 ...
	 */
	@SuppressWarnings( { "unchecked", "deprecation" })
	public static List<WeixinMedia> getAllMedia(String accessToken) {
 		List<WeixinMedia> weixinMediaList = null;
		// 拼接请求地址
		String requestUrl = "https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token=ACCESS_TOKEN";
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken);
		String jsonData = "{\"type\":\"news\",\"offset\":0,\"count\":20}";
		// 查询分组
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", jsonData);

		if (null != jsonObject) {
			try {
				weixinMediaList = JSONArray.toList(jsonObject.getJSONArray("item"), WeixinMedia.class);
			} catch (JSONException e) {
				weixinMediaList = null;
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				log.error("查询分组失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return weixinMediaList;
	}
	/**
	 * 下载媒体文件
	 * 
	 ...
	 * 添加客服
	 */
	public static boolean createCustom(String accessToken,WeixinCustom custom){
		boolean result = false;
		
		String requestUrl = "https://api.weixin.qq.com/customservice/kfaccount/add?access_token=ACCESS_TOKEN";
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken);
		// 需要提交的json数据
		String jsonData = "{\"kf_account\":\"%s\",\"nickname\":\"%s\"}";
		// 创建分组
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", String.format(jsonData, custom.getKf_account(),custom.getNickname()));
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = false;
				log.info("添加客服成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				log.error("添加客服失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		
		String requestUrl2 = "https://api.weixin.qq.com/customservice/kfaccount/inviteworker?access_token=ACCESS_TOKEN";
		String jsonData2 = "{\"kf_account\":\"%s\",\"invite_wx\":\"%s\"}";
		JSONObject jsonObject2 = CommonUtil.httpsRequest(requestUrl2, "POST", String.format(jsonData2, custom.getKf_account(),custom.getInvite_wx()));
		if (null != jsonObject2) {
			int errorCode = jsonObject2.getInt("errcode");
			String errorMsg = jsonObject2.getString("errmsg");
			if (0 == errorCode) {
				result = true;
				log.info("客服绑定成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				log.error("客服绑定失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	 ...
	 * @return
	 */
	public static boolean createCustomSession(String accessToken,String openid,String kf_account){
		boolean result = false;
		
		String requestUrl = "https://api.weixin.qq.com/customservice/kfsession/create?access_token=ACCESS_TOKEN";
		requestUrl = requestUrl.replace("ACCESS_TOKEN", accessToken);
		// 需要提交的json数据
		String jsonData = "{\"kf_account\":\"%s\",\"openid\":\"%s\"}";
		// 创建分组
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", String.format(jsonData, kf_account,openid));
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = false;
				log.info("会话创建成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
